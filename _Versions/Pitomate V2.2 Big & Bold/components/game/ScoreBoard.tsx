...
export default function ScoreBoard({
    ...
    return (
        <div className="flex flex-col h-screen max-w-2xl mx-auto bg-[#4A3B32] text-[#FDFBF7] font-hand overflow-hidden relative shadow-2xl">
            <div className="flex justify-between items-center px-6 py-6 border-b border-white/10 bg-[#4A3B32] z-10 shadow-md">
                <div className="flex flex-col items-center w-1/2 border-r border-white/10">
                    <div className="flex flex-col items-center text-xl font-bold text-[#A5D6A7] whitespace-nowrap">
                        <span className="text-2xl">{teamA[0]}</span>
                        <span className="text-2xl font-black mt-1 mb-1 text-[#A5D6A7]">&</span>
                ...
                <div className="flex flex-col items-center w-1/2 pl-4">
                    <div className="flex flex-col items-center text-xl font-bold text-[#EF9A9A] whitespace-nowrap">
                        <span className="text-2xl">{teamB[0]}</span>
                        <span className="text-2xl font-black mt-1 mb-1 text-[#EF9A9A]">&</span>
                ...
                <AnimatePresence>
                    {hands.map((hand, index) => (
                        <motion.div key={hand.handNumber} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between relative">
                            <button onClick={() => selectCell(index, 'A')} className={`w-28 h-20 rounded-xl flex items-center justify-center text-4xl font-bold transition-all ${index === activeCell?.handResultIndex && activeCell?.team === 'A' ? "bg-white text-[#4A3B32] ring-4 ring-[#81C784]" : "bg-white/5 hover:bg-white/10"} ${hand.pointsA === null ? "opacity-50" : "opacity-100"}`}>
                                {hand.pointsA !== null ? hand.pointsA : (index === activeCell?.handResultIndex && activeCell?.team === 'A' ? "_" : "")}
                            </button>
                            <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-xl opacity-30 font-bold">{hand.handNumber}</div>
                            <button onClick={() => selectCell(index, 'B')} className={`w-28 h-20 rounded-xl flex items-center justify-center text-4xl font-bold transition-all ${index === activeCell?.handResultIndex && activeCell?.team === 'B' ? "bg-white text-[#4A3B32] ring-4 ring-[#E57373]" : "bg-white/5 hover:bg-white/10"} ${hand.pointsB === null ? "opacity-50" : "opacity-100"}`}>
            ...
            <AnimatePresence>
                {activeCell && (
                    <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 300 }} className="absolute bottom-0 left-0 right-0 bg-[#FDFBF7] p-4 rounded-t-3xl shadow-2xl z-50 text-[#4A3B32]">
                        <div className="text-center mb-4 text-xl font-bold opacity-50 uppercase">Ingresando puntos para {activeCell.team === 'A' ? "NOSOTROS" : "ELLOS"}</div>
