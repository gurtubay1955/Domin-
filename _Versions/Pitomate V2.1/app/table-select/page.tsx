    ...
    return (
        <div className="min-h-screen bg-[#4A3B32] text-[#FDFBF7] font-hand p-4 pb-20">
                                    ...
                                    >
                                        <div className="flex items-center gap-4 w-full">
                                            ...
                                            <div className="text-left flex-1 min-w-0">
                                                <p className="font-bold text-xl leading-snug truncate">
                                                    {names.join(" y ")}
                                                </p>
                                            </div>
                                            ...
                                            <div className={`
                                                flex-shrink-0 w-14 h-14 flex items-center justify-center rounded-lg border-2
                                                ${isSelected
                    ...
                    <div className="mt-8 pt-8 border-t border-white/5">
                        <div className="flex items-center justify-between gap-4 mb-4">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-xs opacity-40 uppercase tracking-widest hidden sm:block">JUEGOS JUGADOS</span>
                                <div className="w-14 h-14 flex items-center justify-center rounded-lg border border-[#A5D6A7]/30 bg-[#A5D6A7]/10 text-[#A5D6A7]">
                                    <span className="text-xl font-bold font-mono">
                                    ...
                                    <div key={idx} className="bg-black/20 p-3 rounded-xl border border-white/5 flex flex-col gap-2 relative overflow-hidden">
                                        ...
                                        <div className="flex items-center justify-between gap-2">
                                            ...
                                            <div className="flex items-center gap-2 flex-1 min-w-0 overflow-hidden">
                                                <div className="flex flex-col text-left leading-tight min-w-0 pr-1">
                                                    {winnerNames.map((name, i) => (
                                                        <span key={i} className="text-xl font-bold text-[#A5D6A7] drop-shadow-md truncate block">
                                                            {name}
                                                        </span>
                                                    ))}
                                                </div>
                                                <span className="text-2xl font-black text-[#A5D6A7] whitespace-nowrap">{winnerScore}</span>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-2 flex-1 justify-end min-w-0 overflow-hidden">
                                                <div className="flex flex-col text-right leading-tight min-w-0 pl-1">
                                                    {loserNames.map((name, i) => (
                                                        <span key={i} className="text-xl font-bold text-[#EF9A9A] drop-shadow-md truncate block">
                                                            {name}
                                                        </span>
                                                    ))}
                                                </div>
                                                ...
                                                <div className="flex flex-col items-center">
                                                    <span className="text-2xl font-black text-[#EF9A9A]">{loserScore}</span>
                                                </div>
            ...
            <div className="mt-12 text-center max-w-lg mx-auto">
                <p className="text-sm opacity-50 uppercase tracking-[0.2em] mb-3">Progreso Global de la Jornada</p>
                <div className="w-full h-8 bg-white/5 rounded-xl overflow-hidden border border-white/10 relative">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                        <p className="text-sm font-bold text-[#1B5E20] drop-shadow-sm">
                            {Math.round((matchHistory.length / totalExpectedTournament) * 100)}%
                        </p>
                    </div>
                </div>
                <p className="text-xl font-bold mt-4 text-[#A5D6A7]">{matchHistory.length} / {totalExpectedTournament} Partidas</p>
